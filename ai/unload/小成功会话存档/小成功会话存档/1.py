...
class WeWorkFinanceSDK:
    ...
    def test_image_download(self):
        ...
        try:
            ret = self.lib.Init(sdk, self.corpid.encode('utf-8'), self.chat_secret.encode('utf-8'))
            if ret != 0:
                print(f"初始化 SDK 失败, 返回值: {ret}")
                return
            
            # 使用您提供的sdkfileid
            test_sdkfileid = ""
            
            print("=== 开始下载 ===")
            result = self.download_media(sdk, test_sdkfileid)
            
            if result:
                print(f"文件保存到: {result}")
                try:
                    # 尝试用默认程序打开文件
