...
class CSVToWeChatCombinedImporter:
    def __init__(self):
        ...
        # 日期配置
        self.target_date = datetime(2025, 8, 27)  # 目标日期，可根据实际情况调整
        ...
        self.week_start_date = self.target_date - timedelta(days=7)  # 一周前
        ...
        try:
            ...
            for record in result.get("data", []):
                ...
                historical_data[userid] = {
                    ...
                    "累计消息发送数": self._extract_number_value(values.get("累计消息发送数", 0)),
    ...
    def process_individual_data(self, csv_data: List[Dict]) -> List[Dict]:
        ...
        # 批量查询历史数据
        logger.info("开始批量查询历史数据用于计算")
        prev_cum_data = self.batch_query_historical_data(user_ids, self.prev_timestamp)
        weekly_data = self.query_weekly_data(user_ids)
        ...
        for row in csv_data:
            ...
            today_group_members = self._extract_number_value(row.get("截至当天客户群人数", 0))
            today_group_msg = self._extract_number_value(row.get("截至当天客户群消息数", 0))

            # 2. 获取历史累积值（默认0）
            prev_cum = prev_cum_data.get(userid, {
                "累积新增客户数": 0,
                "累计消息发送数": 0,
                "累积客户群人数": 0,
                "累积群消息数": 0
            ...
            # 3. 计算累积字段
            cumulative_new = prev_cum["累积新增客户数"] + today_new
            cumulative_msg = prev_cum["累计消息发送数"] + today_msg
            cumulative_group_members = prev_cum["累积客户群人数"] + today_group_members
            cumulative_group_msg = prev_cum["累积群消息数"] + today_group_msg

            # 4. 获取周数据
            weekly_new = weekly_data.get(userid, 0) + today_new  # 加上当天的新增
    ...
    def update_hospital_data(self, hospital_aggregations: Dict) -> bool:
        ...
        logger.info(f"查询医院近一周数据: {hospital_ids}")
        ...
        for hospital_id, agg_data in hospital_aggregations.items():
            ...
            update_data = {
                ...
                "WordList": {
                    ...
                    "record_id": record_id,
                    "values": {
                        "新增客户数": agg_data["新增客户数"],
                        "发送消息数": agg_data["发送消息数"],
...
if __name__ == "__main__":
    # 直接指定CSV文件路径
    CSV_FILE_PATH = "wechat_stats_20250827.csv"  # 这里填写你的CSV文件名
