...
from model.hypergraph_transformer import HypergraphTransformer 
...
class AMNTDDA(nn.Module):
    def __init__(self, args):
        ...
        #add dwp 2026-1-29
        # 假设疾病特征是 One-hot 或其他维度，这里添加线性层确保对齐
        # 如果 args.disease_number 就是疾病特征维度，则保留；否则请根据实际情况修改
        self.disease_linear = nn.Linear(args.disease_number, args.hgt_in_dim) # 假设疾病用One-hot或类似特征，根据实际情况调整输入维度
        # # 初始化药物图 Transformer：处理药物-药物相似性图
        # self.gt_drug = gt_net_drug.GraphTransformer(device, args.gt_layer, args.drug_number, args.gt_out_dim,
        #                                             args.gt_out_dim,
        #                                             args.gt_head, args.amdgt_dropout)
        # print("=== GraphTransformer 配置参数 ===")
        # print(f"Device: {device}")
        # print(f"Layers (层数): {args.gt_layer}")
        # print(f"Drug Number (节点数): {args.drug_number}")
        # print(f"Out Dim (输出维度): {args.gt_out_dim}")
        # print(f"Heads (注意力头数): {args.gt_head}")
        # print(f"Dropout: {args.amdgt_dropout}")
        # print("================================")
        """=== GraphTransformer 配置参数 ===
        Device: cuda
        Layers (层数): 2
        Drug Number (节点数): 663
        Out Dim (输出维度): 200
        ...
        # ============================================================
        # 目的：替代原来的 GraphTransformer，直接利用超图结构 H 提取特征
        # 借鉴思路：Node -> Hyperedge -> Node 的信息聚合流
        
        # 药物超图 Transformer
        self.hypergt_drug = HypergraphTransformer(
            in_dim=args.hgt_in_dim,       # 输入维度 (投影后的特征维度)#64
            hidden_dim=args.gt_out_dim,   # 隐藏层维度 200
            out_dim=args.gt_out_dim,      # 输出维度 200
            num_layers=args.gt_layer,     # 层数 2 
            num_heads=args.gt_head,       # 多头数量 2
            dropout=args.amdgt_dropout    #0.2
        )

        # 疾病超图 Transformer
        # 注意：这里输入维度也是 hgt_in_dim，因为我们后面会对疾病特征也做 linear 投影
        # 如果疾病特征维度不同，请调整 in_dim
        self.hypergt_disease = HypergraphTransformer(
            in_dim=args.hgt_in_dim,  #64
            hidden_dim=args.gt_out_dim, #200
            out_dim=args.gt_out_dim, #200
            num_layers=args.gt_layer, #2
        ...
        # 【新增】维度对齐层
        # 计算 HGT 最后一层的实际输出维度
        hgt_actual_out = args.hgt_head_dim * args.hgt_head
        # 将其投影到 gt_out_dim，确保能和 HyperGT 输出拼接
        self.hgt_to_gt_proj = nn.Linear(hgt_actual_out, args.gt_out_dim)
    ...
    def forward(self, drdr_graph, didi_graph, drdipr_graph, drug_feature, disease_feature, protein_feature, sample,
                H_drdr, H_didi):
        # print("DEBUG: Graph Ntypes:", drdipr_graph.ntypes)
        # dr_sim = self.gt_drug(drdr_graph)
        # di_sim = self.gt_disease(didi_graph)
        # #1. 提取相似性图特征-捕捉同类节点内部的拓扑关系（例如：结构相似的药物可能有相似的功能）
        # dr_sim = self.gt_drug(drdr_graph, H_drdr)
        # di_sim = self.gt_disease(didi_graph, H_didi)
        ...
        # protein_feature = self.protein_linear(protein_feature)
          ...
          # === 1. 特征预处理与投影 ===
        # 将原始特征映射到统一维度
        drug_emb = self.drug_linear(drug_feature)
        protein_emb = self.protein_linear(protein_feature)
        
        # 疾病特征投影 (如果维度不匹配)
        if disease_feature.shape[1] != self.args.hgt_in_dim:
             disease_emb = self.disease_linear(disease_feature.float())
        else:
             disease_emb = disease_feature# [N_dis, 64]

        # ============================================================
        # 【关键修改】 超图 Transformer 提取特征
        # ============================================================
        # 输入：投影后的节点特征, 超图关联矩阵 (H_drdr)
        # 逻辑：HyperGT 利用 H 矩阵实现 Node->Hyperedge->Node 的特征提取
        # 输出：[N, gt_out_dim] 的嵌入矩阵
        dr_hyper = self.hypergt_drug(drug_emb, H_drdr)       
        di_hyper = self.hypergt_disease(disease_emb, H_didi) 
        # ============================================================
        ...
        # 【新增】维度对齐：HGT输出 -> GT输出维度
        # 确保 HGT 的特征维度 (可能是 25*8=200) 与 HyperGT (200) 一致
        if hasattr(self, 'hgt_to_gt_proj'):
            h_hgt = self.hgt_to_gt_proj(h_hgt)    
        ...
        dr = torch.stack((dr_hyper, dr_hgt), dim=1)
        di = torch.stack((di_hyper, di_hgt), dim=1)
